...
def init_db():
    ...
    # إصلاح روابط الصور القديمة التي كانت مكتوبة بشكل خاطئ مثل %sw=900
    ...
    # إجبار الأصناف الأساسية على صور مختلفة وصحيحة حتى لو كانت قاعدة البيانات القديمة حفظت صورة مكررة أو رابط تالف
...
def home():
    ...
    return render_template_string(HOME_HTML, menu=menu_items, qr_url=f"{base_url}/qr", menu_url=f"{base_url}/")
...
<main class="wrap"><section class="hero"><div><small>مرحباً بك في</small><h2>كافيه فرفشة 👋</h2><p>اختار طلبك وحدد مكان التوصيل</p></div></section><nav class="tabs"><button class="tab active" onclick="filterCategory('all',this)">الكل</button><button class="tab" onclick="filterCategory('مشروبات ساخنة',this)">☕ ساخنة</button><button class="tab" onclick="filterCategory('مشروبات باردة',this)">🥤 باردة</button><button class="tab" onclick="filterCategory('شيشة',this)">💨 شيشة</button><button class="tab" onclick="filterCategory('حلويات',this)">🍰 حلويات</button><button class="tab" onclick="filterCategory('وجبات خفيفة',this)">🥪 خفيفة</button></nav><section class="products">{% for item in menu %}<article class="product product-card" data-category="{{ item.category }}"><div class="photo"><img src="{{ item.image }}" alt="{{ item.name }}" loading="lazy" onerror="this.onerror=null;this.src='{{ default_image if default_image is defined else 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=900' }}';"></div><div class="info"><h3>{{ item.name }}</h3><p class="desc">{{ item.description }}</p><div class="bottom"><div class="price">{{ item.price }} جنيه</div><div class="qty"><button onclick="changeQty({{ item.id }},-1)">-</button><b id="qty-{{ item.id }}">0</b><button onclick="changeQty({{ item.id }},1)">+</button><button class="add" onclick="addToCart({{ item.id }})">🛒</button></div></div></div></article>{% endfor %}</section></main>
...
function setCartOpen(isOpen){const list=document.getElementById('cartList');const btn=document.getElementById('cartToggleBtn');if(isOpen){list.classList.add('open');if(btn)btn.innerText='عرض & إخفاء السلة';}else{list.classList.remove('open');if(btn)btn.innerText='عرض & إخفاء السلة';}}
function toggleCart(){const list=document.getElementById('cartList');setCartOpen(!list.classList.contains('open'))}
