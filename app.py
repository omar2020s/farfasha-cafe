...
def log_action(action):
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("INSERT INTO activity_log (user_name, action, created_at) VALUES (?, ?, ?)", ("مدير", action, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def init_db():
    ...
    if cur.fetchone()["c"] == 0:
        cur.executemany(
            "INSERT INTO menu_items (id, name, description, price, category, image, available, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def send_order():
    ...
    cur.execute(
        "INSERT INTO orders (table_number, order_place, customer_name, total, status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        (order_place, order_place, customer_name, total, "جديد", created_at)
    )
    order_id = cur.lastrowid
    for item in clean_items:
        cur.execute("INSERT INTO order_items (order_id, item_name, qty, price) VALUES (?, ?, ?, ?)", (order_id, item["name"], item["qty"], item["price"]))
    cur.execute("INSERT INTO notifications (message, created_at) VALUES (?, ?)", (f"طلب جديد رقم #{order_id} من {customer_name} - {order_place}", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    cur.execute("INSERT INTO activity_log (user_name, action, created_at) VALUES (?, ?, ?)", (customer_name, f"إضافة طلب رقم #{order_id}", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()
    conn.close()
...
def admin():
    ...
    cur.execute("SELECT COUNT(*) AS c FROM orders WHERE status = 'تم التسليم' AND created_at LIKE ?", (today + "%",))
    completed_today = cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) AS c FROM orders WHERE status = 'جديد'")
    waiting_orders = cur.fetchone()["c"]
    cur.execute("SELECT SUM(total) AS s FROM orders WHERE status != 'ملغي' AND created_at LIKE ?", (today + "%",))
    sales_today = cur.fetchone()["s"] or 0
    cur.execute("SELECT SUM(total) AS s FROM orders WHERE status != 'ملغي' AND created_at LIKE ?", (month + "%",))
    sales_month = cur.fetchone()["s"] or 0
    cur.execute("SELECT COUNT(DISTINCT order_place) AS c FROM orders WHERE created_at LIKE ?", (today + "%",))
...
def update_status(order_id, status):
    ...
    cur.execute("INSERT INTO activity_log (user_name, action, created_at) VALUES (?, ?, ?)", ("مدير", f"تغيير حالة الطلب #{order_id} إلى {status}", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()
    conn.close()
...
def delete_order(order_id):
    ...
    cur.execute("INSERT INTO activity_log (user_name, action, created_at) VALUES (?, ?, ?)", ("مدير", f"حذف الطلب #{order_id}", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()
    conn.close()
    return redirect(url_for("admin") + "#orders")
...
def menu_add():
    ...
    if name and price > 0:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("INSERT INTO menu_items (name, description, price, category, image, available, created_at) VALUES (?, ?, ?, ?, ?, 1, ?)", (name, description, price, category, image, datetime.now().strftime("%Y-%m-%d %H:%M")))
        conn.commit()
        conn.close()
...
def inventory_add():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("INSERT INTO inventory (item_name, quantity, unit, min_quantity, updated_at) VALUES (?, ?, ?, ?, ?)", (
...
def employees_add():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("INSERT INTO employees (name, role, active, created_at) VALUES (?, ?, 1, ?)", (request.form.get("name", "موظف"), request.form.get("role", "عامل كافيه"), datetime.now().strftime("%Y-%m-%d %H:%M")))
    conn.commit()
    conn.close()
