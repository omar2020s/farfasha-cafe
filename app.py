...
function filterCategory(c,b){
    // إصلاح فلتر التصنيفات بعد استخدام بحث الصنف
    // نمسح تأثير البحث أولاً حتى لا يمنع ظهور الأصناف
    const searchInput=document.getElementById('productSearchInput');
    const searchBox=document.getElementById('productSearchResults');
    if(searchInput) searchInput.value='';
    if(searchBox){searchBox.classList.remove('show');searchBox.innerHTML='';}

    document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));
    if(b) b.classList.add('active');

    const wanted=normalizeText(c);
    document.querySelectorAll('.product-card').forEach(card=>{
        card.classList.remove('hide-by-search');
        const cardCategory=normalizeText(card.dataset.category||'');
        card.style.display=(c==='all'||cardCategory===wanted)?'grid':'none';
    });
}
