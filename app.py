...
function sendOrder(){if(cart.length===0){showToast('السلة فارغة');return;}fetch('/send-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({table_number:tableNumber,cart:cart})}).then(r=>r.json()).then(data=>{showToast(data.message);if(data.success){alert('✅ تم إرسال الطلب بنجاح إلى الإدارة');clearCart();document.getElementById('cartList').classList.remove('open')}})}
renderCart();
</script>
</body></html>
'''
