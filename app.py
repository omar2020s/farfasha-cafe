...
def send_order():
    ...
    cur.execute("INSERT INTO orders (table_number, total, status, created_at) VALUES (?, ?, ?, ?)",
                (table_number, total, "قيد التحضير", created_at))
    order_id = cur.lastrowid
    for item in clean_items:
        cur.execute("INSERT INTO order_items (order_id, item_name, qty, price) VALUES (?, ?, ?, ?)",
...
</div><div class="floating-cart"><div><b>الإجمالي</b><br><span id="floatTotal">0 جنيه</span></div><button onclick="document.getElementById('cartBox').scrollIntoView({behavior:'smooth'})">عرض السلة</button></div>
<script>
const menu={{ menu|tojson }}; const tableNumber="{{ table_number }}"; let quantities={}; let cart=[];
function filterCategory(category,btn){document.querySelectorAll('.cat-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');document.querySelectorAll('.product-card').forEach(card=>{card.style.display=(category==='all'||card.dataset.category===category)?'grid':'none';});}
...
function addToCart(id){let qty=quantities[id]||0;if(qty<=0){alert('اختار الكمية أولاً');return;}let item=menu.find(x=>x.id===id);let exists=cart.find(x=>x.id===id);if(exists){exists.qty+=qty}else{cart.push({id:item.id,name:item.name,price:item.price,qty:qty})}quantities[id]=0;document.getElementById('qty-'+id).innerText=0;renderCart();}
function renderCart(){let box=document.getElementById('cartItems');let total=0;box.innerHTML='';if(cart.length===0)box.innerHTML='<p style="color:#7d6f67">لا يوجد طلبات حالياً</p>';cart.forEach((item,index)=>{total+=item.price*item.qty;box.innerHTML+=`<div class="cart-item"><div>${item.name}</div><b>${item.qty}</b><div>${item.price*item.qty} جنيه</div><button class="remove" onclick="removeItem(${index})">×</button></div>`});document.getElementById('totalPrice').innerText=total;document.getElementById('floatTotal').innerText=total+' جنيه';}
function removeItem(index){cart.splice(index,1);renderCart();} function clearCart(){cart=[];renderCart();}
function sendOrder(){if(cart.length===0){alert('السلة فارغة');return;}fetch('/send-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({table_number:tableNumber,cart:cart})}).then(r=>r.json()).then(data=>{alert(data.message);if(data.success)clearCart();});}
renderCart();
</script></body></html>
'''
