...
def init_db():
    ...
    """)
    # Migration for old SQLite databases: add customer_name if missing
...
def send_order():
    ...
    cur.execute(
        "INSERT INTO orders (table_number, total, status, created_at, customer_name) VALUES (?, ?, ?, ?, ?)",
        (table_number, total, "قيد التحضير", created_at, customer_name),
    )
...
function sendOrder(){
    ...
    fetch('/send-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({table_number:tableNumber,customer_name:customerName,cart:cart})}).then(r=>r.json()).then(data=>{showToast(data.message);if(data.success){alert('✅ تم إرسال الطلب باسم: '+customerName);clearCart();if(customerNameInput){customerNameInput.value='';}document.getElementById('cartList').classList.remove('open')}})
}
renderCart();
</script>
</body></html>
