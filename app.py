...
ORDER_PLACES = ["النيابة الادارية"] + [f"طاولة رقم {i}" for i in range(1, 16)]
...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS orders (
            ...
            order_place TEXT DEFAULT 'النيابة الادارية',
    ...
    ensure_column(cur, "orders", "customer_name", "TEXT DEFAULT 'عميل'")
    ensure_column(cur, "orders", "order_place", "TEXT DEFAULT 'النيابة الادارية'")
    cur.execute(
        """
...
def admin():
    ...
    for order in orders_rows:
        ...
        order_place = order["order_place"] if "order_place" in order.keys() else "النيابة الادارية"
        customer_name = order["customer_name"] if "customer_name" in order.keys() else "عميل"
        orders.append({
            "id": order["id"],
            "order_place": order_place or "النيابة الادارية",
    ...
    cur.execute("SELECT COUNT(DISTINCT COALESCE(order_place, 'النيابة الادارية')) AS count FROM orders WHERE created_at LIKE ?", (today + "%",))
...
def update_status(order_id, status):
    ...
    conn.commit()
    conn.close()
    session["admin_notice"] = f"تم تحديث الطلب رقم #{order_id} إلى: {status}"
    return redirect(url_for("admin"))
...
<main class="wrap">
<section class="hero"><div><small>مرحباً بك في</small><h2>كافيه فرفشة 👋</h2><p>لذيذ يبدأ من هنا ✨</p><div class="place-chip">اختر مكان الطلب من السلة</div></div></section>
...
function addToCart(id){let qty=quantities[id]||0;if(qty<=0){showToast('اختار الكمية أولاً');return;}let item=menu.find(x=>x.id===id);let exists=cart.find(x=>x.id===id);if(exists){exists.qty+=qty}else{cart.push({id:item.id,name:item.name,price:item.price,qty:qty})}quantities[id]=0;document.getElementById('qty-'+id).innerText=0;renderCart();document.getElementById('cartList').classList.add('open');showToast('تمت الإضافة إلى السلة')}
...
function sendOrder(){if(cart.length===0){showToast('السلة فارغة');return;}const customerNameInput=document.getElementById('customerName');const orderPlaceInput=document.getElementById('orderPlace');const customerName=customerNameInput.value.trim();const orderPlace=orderPlaceInput.value;if(!customerName){document.getElementById('cartList').classList.add('open');customerNameInput.focus();showToast('من فضلك اكتب اسم صاحب الطلب');return;}if(!orderPlace){document.getElementById('cartList').classList.add('open');orderPlaceInput.focus();showToast('من فضلك اختر مكان الطلب');return;}fetch('/send-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({customer_name:customerName,order_place:orderPlace,cart:cart})}).then(r=>r.json()).then(data=>{showToast(data.message);if(data.success){alert('✅ تم إرسال الطلب باسم: '+customerName+'\n📍 مكان الطلب: '+orderPlace);clearCart();customerNameInput.value='';orderPlaceInput.value='النيابة الادارية';document.getElementById('cartList').classList.remove('open')}})}
renderCart();
</script>
</body></html>
'''
