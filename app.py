...
DEFAULT_MENU = [
    (1, "قهوة تركي", "قهوة تركية أصلية بطعم غني ورائحة مميزة", 15, "مشروبات ساخنة", "https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?w=900", 1),
    (2, "شاي", "شاي أحمر نقي ومنعش", 10, "مشروبات ساخنة", "https://images.unsplash.com/photo-1576092768241-dec231879fc3?w=900", 1),
    (3, "كابتشينو", "اسبريسو مع حليب مبخر ورغوة ناعمة", 25, "مشروبات ساخنة", "https://images.unsplash.com/photo-1534778101976-62847782c213?w=900", 1),
    (4, "موكا", "مزيج رائع من القهوة والشوكولاتة", 30, "مشروبات ساخنة", "https://images.unsplash.com/photo-1572442388796-11668a67e53d?w=900", 1),
    (5, "قهوة مثلجة", "قهوة باردة ومنعشة", 25, "مشروبات باردة", "https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=900", 1),
    (6, "عصير برتقال", "عصير طبيعي طازج", 20, "مشروبات باردة", "https://images.unsplash.com/photo-1621506289937-a8e4df240d0b?w=900", 1),
    (7, "كيك شوكولاتة", "قطعة كيك غنية بالشوكولاتة", 30, "حلويات", "https://images.unsplash.com/photo-1606890737304-57a1ca8a5b62?w=900", 1),
    (8, "ساندوتش دجاج", "ساندوتش دجاج طازج", 35, "وجبات خفيفة", "https://images.unsplash.com/photo-1553909489-cd47e0907980?w=900", 1),
]
...
def get_db():
    if not DATABASE_URL:
        raise RuntimeError("DATABASE_URL is missing. Add PostgreSQL DATABASE_URL in Render Environment Variables.")
...
def log_action(action, user_name="مدير"):
    try:
        ...
        cur.execute(
            ...
            (user_name, action, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )
        conn.commit()
        cur.close()
        conn.close()
...
def init_db():
    ...
    """)
    for cid, name, icon, active, sort_order in DEFAULT_CATEGORIES:
        cur.execute(
            """
            INSERT INTO categories (id, name, icon, active, sort_order, created_at)
            VALUES (%s, %s, %s, %s, %s, %s)
            ON CONFLICT (name) DO UPDATE SET icon = EXCLUDED.icon
            """,
            (cid, name, icon, active, sort_order, datetime.now().strftime("%Y-%m-%d %H:%M")),
        )
    cur.execute("SELECT setval(pg_get_serial_sequence('categories','id'), COALESCE((SELECT MAX(id) FROM categories), 1), true)")
    ...
    cur.execute("UPDATE menu_items SET image = REPLACE(image, '%%sw=900', '?w=900') WHERE image LIKE '%%%%sw=900%%'")
    cur.execute("UPDATE menu_items SET image = %s WHERE image IS NULL OR image = ''", (DEFAULT_IMAGE,))
    for item_id, name, description, price, category, image, available in DEFAULT_MENU:
        cur.execute(
            """
            UPDATE menu_items SET image = %s,
                   description = COALESCE(NULLIF(description,''), %s),
                   category = COALESCE(NULLIF(category,''), %s)
            WHERE id = %s
            """,
            (image, description, category, item_id),
        )
...
def build_order_message(order_id, customer_name, order_place, clean_items, total):
    items_text = "\n".join([f"☕ {it['name']} × {it['qty']} = {it['qty'] * it['price']} جنيه" for it in clean_items])
    return f"""🔔 طلب جديد من كافيه فرفشة
...
def send_order():
    try:
        ...
        if not customer_name:
            return jsonify({"success": False, "message": "من فضلك اكتب اسم صاحب الطلب"}), 400
        if not order_place:
            return jsonify({"success": False, "message": "من فضلك اختر مكان الطلب"}), 400
        if not cart:
        ...
        for cart_item in cart:
            ...
            if item and qty > 0:
                ...
                total += price * qty
                clean_items.append({"name": item["name"], "qty": qty, "price": price})

        if not clean_items:
            return jsonify({"success": False, "message": "لا توجد عناصر صحيحة"}), 400
        ...
        for item in clean_items:
            cur.execute(
                "INSERT INTO order_items (order_id, item_name, qty, price) VALUES (%s, %s, %s, %s)",
                (order_id, item["name"], item["qty"], item["price"]),
            )
        cur.execute(
            "INSERT INTO notifications (message, created_at) VALUES (%s, %s)",
            (f"طلب جديد رقم #{order_id} من {customer_name} - {order_place}", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )
        cur.execute(
            "INSERT INTO activity_log (user_name, action, created_at) VALUES (%s, %s, %s)",
            (customer_name, f"إضافة طلب رقم #{order_id}", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )
        conn.commit()
        cur.close()
        conn.close()

        tg_result = send_telegram_message(build_order_message(order_id, customer_name, order_place, clean_items, total))
        if tg_result.get("ok"):
            log_action(f"تم إرسال الطلب #{order_id} إلى Telegram")
        else:
            log_action(f"فشل إرسال Telegram للطلب #{order_id}: {tg_result}")

        return jsonify({"success": True, "message": "تم إرسال الطلب بنجاح ✅", "order_id": order_id, "total": total})
    except Exception as e:
        return jsonify({"success": False, "message": f"حدث خطأ أثناء إرسال الطلب: {str(e)}"}), 500


@app.route("/test-telegram")
def test_telegram():
    result = send_telegram_message("✅ اختبار Telegram من كافيه فرفشة")
...
def admin():
    ...
    cur.execute("SELECT COALESCE(SUM(total),0) AS s FROM orders WHERE status != 'ملغي' AND created_at LIKE %s", (today + "%",))
    sales_today = float(cur.fetchone()["s"] or 0)
    cur.execute("SELECT COALESCE(SUM(total),0) AS s FROM orders WHERE status != 'ملغي' AND created_at LIKE %s", (month + "%",))
    sales_month = float(cur.fetchone()["s"] or 0)
    cur.execute("SELECT COUNT(DISTINCT order_place) AS c FROM orders WHERE created_at LIKE %s", (today + "%",))
    active_places = cur.fetchone()["c"]
    cur.execute("SELECT item_name, SUM(qty) AS qty FROM order_items GROUP BY item_name ORDER BY qty DESC LIMIT 1")
    top_row = cur.fetchone()
    top_item = top_row["item_name"] if top_row else "لا يوجد"
    cur.execute("SELECT item_name, SUM(qty) AS qty FROM order_items GROUP BY item_name ORDER BY qty DESC LIMIT 5")
    top_items = [dict(r) for r in cur.fetchall()]
    cur.execute("SELECT item_name, SUM(qty) AS qty FROM order_items GROUP BY item_name ORDER BY qty ASC LIMIT 5")
...
def category_add():
    ...
    if name:
        ...
        cur.execute(
            """
            INSERT INTO categories (name, icon, active, sort_order, created_at)
            VALUES (%s, %s, 1, %s, %s)
            ON CONFLICT (name) DO UPDATE SET icon = EXCLUDED.icon, active = 1, sort_order = EXCLUDED.sort_order
            """,
            (name, icon, sort_order, datetime.now().strftime("%Y-%m-%d %H:%M")),
        )
        conn.commit()
        cur.close()
        conn.close()
        log_action(f"إضافة/تعديل تصنيف {name}")
...
def category_delete(category_id):
    ...
    row = cur.fetchone()
    if row:
        cur.execute("SELECT COUNT(*) AS c FROM menu_items WHERE category = %s", (row["name"],))
        used = cur.fetchone()["c"]
        if used == 0:
...
def menu_add():
    ...
    image = request.form.get("image", "").strip() or DEFAULT_IMAGE
    if name and price > 0:
        conn = get_db()
        cur = conn.cursor()
...
function setCartOpen(isOpen){const list=document.getElementById('cartList');if(isOpen){list.classList.add('open')}else{list.classList.remove('open')}}function toggleCart(){const list=document.getElementById('cartList');setCartOpen(!list.classList.contains('open'))}
...
function addToCart(id){let q=quantities[id]||0;if(q<=0){toast('اختار الكمية أولاً');return}let item=menu.find(x=>x.id===id);let ex=cart.find(x=>x.id===id);if(ex){ex.qty+=q}else{cart.push({id:item.id,name:item.name,price:parseFloat(item.price),qty:q})}quantities[id]=0;document.getElementById('qty-'+id).innerText=0;renderCart();toast('تمت الإضافة')}
...
function removeItem(i){cart.splice(i,1);renderCart()}function clearCart(){cart=[];renderCart();setCartOpen(false)}
function sendOrder(){if(cart.length===0){toast('السلة فارغة');setCartOpen(true);return}let customerName=document.getElementById('customerName').value.trim();let orderPlace=document.getElementById('orderPlace').value;if(!customerName){setCartOpen(true);document.getElementById('customerName').focus();toast('اكتب اسم صاحب الطلب');return}if(!orderPlace){setCartOpen(true);document.getElementById('placeType').focus();toast('اختر مكان الطلب');return}const sendBtn=document.querySelector('.send');if(sendBtn){sendBtn.disabled=true;sendBtn.innerText='جاري الإرسال...'}fetch('/send-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({customer_name:customerName,order_place:orderPlace,cart:cart})}).then(async r=>{const data=await r.json().catch(()=>({success:false,message:'خطأ في قراءة رد السيرفر'}));if(!r.ok){throw new Error(data.message||'فشل إرسال الطلب')}return data}).then(d=>{toast(d.message||'تم إرسال الطلب بنجاح ✅');if(d.success){alert('✅ تم إرسال الطلب بنجاح');clearCart();document.getElementById('customerName').value='';document.getElementById('placeType').value='';updatePlaceOptions();setCartOpen(false)}}).catch(err=>{toast('حدث خطأ: '+err.message);alert('حدث خطأ أثناء إرسال الطلب: '+err.message)}).finally(()=>{if(sendBtn){sendBtn.disabled=false;sendBtn.innerText='إرسال الطلب'}})}renderCart();
</script></body></html>
'''
...
<section id="orders" class="section"><div class="panel"><h3>إدارة الطلبات</h3><div class="filters"><input id="orderSearch" onkeyup="filterOrders()" placeholder="بحث برقم الطلب أو اسم العميل أو المكان أو الصنف"><select id="placeFilter" onchange="filterOrders()"><option value="">كل الأماكن</option><option>النيابة الإدارية</option><option>الكافيه</option></select><select id="statusFilter" onchange="filterOrders()"><option value="">كل الحالات</option>{% for st in statuses %}<option>{{ st }}</option>{% endfor %}</select></div><button class="btn btn-orange" onclick="showNewOrdersOnly()">عرض الطلبات الجديدة فقط</button> <button class="btn btn-brown" onclick="resetOrderFilters()">عرض كل الطلبات</button><div class="table-wrap"><table id="ordersTable"><thead><tr><th>رقم</th><th>العميل</th><th>المكان</th><th>الوقت</th><th>العناصر</th><th>الإجمالي</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody>{% for o in orders %}<tr data-status="{{ o.status }}"><td>#{{ o.id }}</td><td><b>{{ o.customer_name }}</b></td><td>{{ o.order_place }}</td><td>{{ o.created_at }}</td><td>{% for i in o.order_items %}• {{ i.item_name }} × {{ i.qty }}<br>{% endfor %}</td><td><b>{{ o.total }}</b> جنيه</td><td><span class="badge {% if o.status=='جديد' %}badge-new{% elif o.status=='قيد التحضير' %}badge-work{% elif o.status=='جاهز' %}badge-ready{% elif o.status=='تم التسليم' %}badge-done{% else %}badge-cancel{% endif %}">{{ o.status }}</span></td><td>{% for st in statuses %}<a class="btn btn-blue" href="/update-status/{{ o.id }}/{{ st }}">{{ st }}</a> {% endfor %}<a class="btn btn-brown" target="_blank" href="/invoice/{{ o.id }}">طباعة</a> <a class="btn btn-red" onclick="return confirm('حذف الطلب؟')" href="/delete-order/{{ o.id }}">حذف</a></td></tr>{% endfor %}</tbody></table></div></div></section>
<section id="categories" class="section"><div class="panel"><h3>إدارة التصنيفات</h3><form method="post" action="/category/add" class="grid3"><input name="name" placeholder="اسم التصنيف الجديد مثل: عصائر فريش"><input name="icon" placeholder="أيقونة مثل: 🍹" value="🍽️"><input name="sort_order" type="number" placeholder="الترتيب" value="100"><button class="btn-brown">إضافة / تعديل التصنيف</button></form><div class="table-wrap"><table><thead><tr><th>الأيقونة</th><th>التصنيف</th><th>الترتيب</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>{% for c in categories %}<tr><td>{{ c.icon }}</td><td>{{ c.name }}</td><td>{{ c.sort_order }}</td><td>{{ 'ظاهر' if c.active else 'مخفي' }}</td><td><a class="btn btn-orange" href="/category/toggle/{{ c.id }}">إظهار/إخفاء</a> <a class="btn btn-red" onclick="return confirm('سيتم حذف التصنيف فقط إذا لم يكن مستخدمًا في أي صنف. متابعة؟')" href="/category/delete/{{ c.id }}">حذف</a></td></tr>{% endfor %}</tbody></table></div><p class="muted">بعد إضافة التصنيف سيظهر في المنيو وفي قائمة إضافة صنف جديد.</p></div></section>
<section id="places" class="section"><div class="panel"><h3>إدارة أماكن الطلب</h3><div class="grid2"><div><h4>النيابة الإدارية</h4><div class="mini-card">الدور الأرضي</div><div class="mini-card">الأول علوي</div><div class="mini-card">الثاني علوي</div><div class="mini-card">الثالث علوي</div><div class="mini-card">الرابع علوي</div></div><div><h4>الكافيه</h4>{% for n in range(1,21) %}<span class="badge badge-work">طاولة {{ n }}</span> {% endfor %}</div></div></div></section>
<section id="menu" class="section"><div class="panel"><h3>إدارة المنيو</h3><form method="post" action="/menu/add" class="grid3"><input name="name" placeholder="اسم الصنف"><input name="price" type="number" step="0.01" placeholder="السعر"><select name="category">{% for c in categories %}<option>{{ c.name }}</option>{% endfor %}</select><input name="image" placeholder="رابط صورة الصنف"><textarea name="description" placeholder="وصف الصنف"></textarea><button class="btn-brown">إضافة صنف</button></form><div class="table-wrap"><table><thead><tr><th>الصنف</th><th>التصنيف</th><th>السعر</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>{% for m in menu_items %}<tr><td>{{ m.name }}</td><td>{{ m.category }}</td><td>{{ m.price }}</td><td>{{ 'متاح' if m.available else 'متوقف' }}</td><td><a class="btn btn-orange" href="/menu/toggle/{{ m.id }}">إيقاف/تشغيل</a> <a class="btn btn-red" href="/menu/delete/{{ m.id }}">حذف</a></td></tr>{% endfor %}</tbody></table></div></div></section>
...
function showNewOrdersOnly(){document.getElementById('statusFilter').value='جديد';filterOrders();}
function resetOrderFilters(){document.getElementById('orderSearch').value='';document.getElementById('statusFilter').value='';document.getElementById('placeFilter').value='';filterOrders();}
