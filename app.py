...
def safe_price(value):
    """تحويل السعر إلى رقم بشكل آمن حتى لا يتوقف البرنامج عند إدخال قيمة غير صحيحة."""
    try:
...
def menu_add():
    ...
    price = safe_price(request.form.get("price", 0))
    category = request.form.get("category", "مشروبات ساخنة").strip() or "مشروبات ساخنة"
    ...
    if name and price > 0:
        ...
        cur.execute(
            """
            INSERT INTO menu_items (name, description, price, category, image, available, created_at)
...
def menu_edit(item_id):
    """تعديل بيانات أي صنف موجود في المنيو من لوحة المدير."""
    name = request.form.get("name", "").strip()
    price = safe_price(request.form.get("price", 0))
    category = request.form.get("category", "مشروبات ساخنة").strip() or "مشروبات ساخنة"
    description = request.form.get("description", "").strip()
    image = request.form.get("image", "").strip()
    available = 1 if request.form.get("available", "1") == "1" else 0
    ...
    if name and price > 0:
        ...
        cur.execute(
            ...
            WHERE id = %s
            """,
            (name, description, price, category, image, available, item_id)
        )
        conn.commit()
        cur.close()
        conn.close()
        log_action(f"تعديل صنف {name}")
    return redirect(url_for("admin") + "#menu")
...
<button type="button" class="tab active" data-category="all">الكل</button>
{% for c in categories %}
<button type="button" class="tab" data-category="{{ c.name }}">{{ c.emoji }} {{ c.name }}</button>
{% endfor %}
</nav>
...
function filterCategory(category, btn){
    ...
    const selected = normalizeCategory(category || 'all');
    ...
    document.querySelectorAll('.product-card').forEach(card => {
        ...
        const shouldShow = selected === 'all' || selected === 'الكل' || cardCategory === selected;
...
document.addEventListener('click', function(e){
    const tab = e.target.closest('#categoryTabs .tab');
    if(!tab) return;
    e.preventDefault();
    e.stopPropagation();
    filterCategory(tab.getAttribute('data-category') || 'all', tab);
});
...
<input name="image" placeholder="رابط صورة الصنف">
<textarea name="description" placeholder="وصف الصنف"></textarea>
<button class="btn-brown">إضافة صنف</button>
</form>

<div id="editMenuPanel" class="panel" style="display:none;background:#fffaf6;border:2px solid var(--brown);margin-top:18px">
<h3>✏️ تعديل صنف</h3>
<form method="post" id="editMenuForm" class="grid3">
<input name="name" id="editName" placeholder="اسم الصنف" required>
<input name="price" id="editPrice" type="number" step="0.01" placeholder="السعر" required>
<select name="category" id="editCategory">{% for c in categories %}<option>{{ c }}</option>{% endfor %}</select>
<input name="image" id="editImage" placeholder="رابط صورة الصنف">
<textarea name="description" id="editDescription" placeholder="وصف الصنف"></textarea>
<select name="available" id="editAvailable"><option value="1">متاح</option><option value="0">متوقف</option></select>
<button class="btn-green" type="submit">حفظ التعديل</button>
<button class="btn-red" type="button" onclick="closeEditMenu()">إلغاء</button>
</form>
</div>

<div class="table-wrap"><table><thead><tr><th>الصنف</th><th>الوصف</th><th>التصنيف</th><th>السعر</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>{% for m in menu_items %}<tr><td><b>{{ m.name }}</b></td><td>{{ m.description }}</td><td>{{ m.category }}</td><td>{{ m.price }}</td><td>{{ 'متاح' if m.available else 'متوقف' }}</td><td><button type="button" class="btn btn-blue" onclick='openEditMenu({{ m.id }}, {{ m.name|tojson }}, {{ m.price|string|tojson }}, {{ m.category|tojson }}, {{ m.image|tojson }}, {{ m.description|tojson }}, {{ m.available|tojson }})'>تعديل</button> <a class="btn btn-orange" href="/menu/toggle/{{ m.id }}">إيقاف/تشغيل</a> <a class="btn btn-red" onclick="return confirm('هل تريد حذف هذا الصنف؟')" href="/menu/delete/{{ m.id }}">حذف</a></td></tr>{% endfor %}</tbody></table></div></div></section>
...
function openEditMenu(id,name,price,category,image,description,available){
    const form=document.getElementById('editMenuForm');
    form.action='/menu/edit/'+id;
    document.getElementById('editName').value=name||'';
    document.getElementById('editPrice').value=price||'';
    document.getElementById('editCategory').value=category||'مشروبات ساخنة';
    document.getElementById('editImage').value=image||'';
    document.getElementById('editDescription').value=description||'';
    document.getElementById('editAvailable').value=String(available==1?1:0);
    document.getElementById('editMenuPanel').style.display='block';
    location.hash='menu';
    document.getElementById('editMenuPanel').scrollIntoView({behavior:'smooth',block:'center'});
}
function closeEditMenu(){
    document.getElementById('editMenuPanel').style.display='none';
}
