...
CAFE_TABLES = [f"الكافيه - طاولة رقم {i}" for i in range(1, 21)]
...
def init_db():
    ...
    if "order_place" not in existing_columns:
        cur.execute("ALTER TABLE orders ADD COLUMN order_place TEXT DEFAULT ''")
    if "table_number" not in existing_columns:
        cur.execute("ALTER TABLE orders ADD COLUMN table_number INTEGER DEFAULT 0")
...
def send_order():
    ...
    cur.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (0, order_place, customer_name, total, "قيد التحضير", created_at),
    )
...
def admin():
    ...
    for order in orders_rows:
        ...
        place = order["order_place"] if "order_place" in order.keys() and order["order_place"] else f"طاولة رقم {order['table_number']}"
        customer_name = order["customer_name"] if "customer_name" in order.keys() and order["customer_name"] else "عميل"
        orders.append({
            "id": order["id"],
            "order_place": place,
    ...
    cur.execute("SELECT COUNT(DISTINCT COALESCE(NULLIF(order_place,''), CAST(table_number AS TEXT))) AS count FROM orders WHERE created_at LIKE ?", (today + "%",))
    active_places = cur.fetchone()["count"]
    conn.close()

    admin_notice = session.pop("admin_notice", "")
    base_url = get_site_base_url()
    return render_template_string(
        ADMIN_HTML,
        orders=orders,
...
def update_status(order_id, status):
    ...
    if status == "قيد التحضير":
        session["admin_notice"] = f"تم تحويل الطلب رقم #{order_id} إلى قيد التحضير ✅"
    else:
        session["admin_notice"] = f"تم تنفيذ الطلب رقم #{order_id} ✅"
    return redirect(url_for("admin"))
...
def delete_order(order_id):
    ...
    conn.commit()
    conn.close()
    session["admin_notice"] = f"تم حذف الطلب رقم #{order_id}"
    return redirect(url_for("admin"))
...
<aside class="mobile-menu" id="mobileMenu">
    <button class="close-menu" type="button" id="closeMenuBtn">×</button>
    <h2>☕ كافيه فرفشة</h2>
    <p>نكهة كل لحظة</p>
    <a href="/">🏠 المنيو</a>
    <a href="/login">📋 لوحة المدير</a>
    <a href="{{ qr_url }}" target="_blank">▦ QR للمنيو</a>
    <a href="#cartBox" id="cartMenuLink">🛒 السلة</a>
</aside>
<aside class="desktop-sidebar"><h2>كافيه فرفشة ☕</h2><p>نكهة كل لحظة</p><a class="active" href="/">🏠 المنيو</a><a href="/login">📋 لوحة المدير</a><a href="{{ qr_url }}" target="_blank">▦ QR للمنيو</a></aside>
...
<main class="wrap">
<section class="hero"><div><small>مرحباً بك في</small><h2>كافيه فرفشة 👋</h2><p>اختار مكان الطلب من السلة ✨</p><div class="place-chip">مكان الطلب <b>نيابة / كافيه</b></div></div></section>
...
ADMIN_HTML = r'''
<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>لوحة المدير</title><meta name="viewport" content="width=device-width, initial-scale=1"><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800;900&display=swap" rel="stylesheet"><style>:root{--brown:#3b1d0f;--line:#eee1d7;--muted:#7d6f67;--green:#209653;--red:#c0392b;--orange:#c47b13;--shadow:0 16px 45px rgba(55,30,15,.10)}*{box-sizing:border-box}body{margin:0;background:linear-gradient(180deg,#fffaf4,#f5efe8);font-family:'Cairo',Tahoma,sans-serif;color:#24150d}.layout{display:grid;grid-template-columns:250px 1fr;min-height:100vh}.side{background:linear-gradient(180deg,#251107,#120804);color:#fff;padding:22px;position:sticky;top:0;height:100vh}.brand{display:flex;gap:12px;align-items:center;margin-bottom:30px}.brand .icon{width:52px;height:52px;border-radius:18px;background:rgba(255,255,255,.08);display:grid;place-items:center;font-size:30px}.brand h1{font-size:22px;color:#ffc17d;margin:0}.brand p{margin:0;color:#ccb8a7;font-size:12px}.nav a{display:flex;align-items:center;gap:12px;text-decoration:none;color:#fff;padding:14px;border-radius:16px;margin-bottom:8px;font-weight:800}.nav a.active,.nav a:hover{background:rgba(198,132,63,.28)}.main{padding:26px;min-width:0}.top{height:70px;background:#fff;border:1px solid rgba(90,45,21,.06);border-radius:24px;box-shadow:var(--shadow);display:flex;align-items:center;justify-content:space-between;padding:0 22px;margin-bottom:24px}.top h2{margin:0;font-size:28px}.top-actions{display:flex;gap:10px}.top-actions a{text-decoration:none;background:#f4eee8;color:#3a2518;border-radius:14px;padding:11px 15px;font-weight:800}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.stat{background:#fff;border-radius:24px;padding:22px;box-shadow:var(--shadow);border:1px solid rgba(90,45,21,.06);min-height:150px}.stat .circle{width:56px;height:56px;border-radius:50%;display:grid;place-items:center;background:#fff2e8;font-size:27px;margin-bottom:12px}.stat b{font-size:32px;color:#5a2d15}.stat p{margin:6px 0;color:var(--muted);font-weight:700}.links{margin:18px 0;background:#fff7ef;border:1px solid var(--line);border-radius:22px;padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:12px}.links a{color:#5a2d15;font-weight:800;word-break:break-word}.panel{background:#fff;border-radius:26px;box-shadow:var(--shadow);border:1px solid rgba(90,45,21,.06);padding:22px;margin-top:18px}.panel h3{margin:0 0 4px;font-size:24px}.filters{display:flex;gap:10px;margin:14px 0}.filters input,.filters select{border:1px solid var(--line);border-radius:14px;padding:12px 14px;font-family:inherit;min-width:190px}.table-wrap{overflow:auto;border-radius:18px;border:1px solid var(--line)}table{width:100%;border-collapse:collapse;background:#fff;min-width:980px}th{background:linear-gradient(135deg,#693519,#3b1d0f);color:#fff;padding:15px;text-align:right}td{padding:15px;border-bottom:1px solid #f0e5dc;vertical-align:top}.badge{display:inline-flex;align-items:center;gap:6px;border-radius:12px;padding:7px 11px;font-weight:800;font-size:13px}.wait{background:#fff0d2;color:#b8730c}.done{background:#e5f7ed;color:#148346}.actions{display:flex;gap:7px;flex-wrap:wrap}.btn{text-decoration:none;border-radius:12px;padding:9px 12px;color:#fff;font-weight:800;display:inline-block}.b-orange{background:var(--orange)}.b-green{background:var(--green)}.b-red{background:var(--red)}.mobile-title{display:none}@media(max-width:1000px){.layout{display:block}.side{display:none}.main{padding:14px}.mobile-title{display:block;position:sticky;top:0;z-index:10;background:rgba(255,250,244,.92);backdrop-filter:blur(12px);padding:15px 10px;border-bottom:1px solid #eee}.mobile-title h1{margin:0;font-size:24px}.top{display:none}.grid{grid-template-columns:1fr 1fr}.links{grid-template-columns:1fr}.panel{padding:14px;border-radius:22px}.filters{display:block}.filters input,.filters select{width:100%;margin-bottom:8px}.stat{min-height:130px}.stat b{font-size:28px}}@media(max-width:560px){.grid{grid-template-columns:1fr}.main{padding:10px}.stat{display:flex;align-items:center;gap:18px;min-height:110px}.stat .circle{margin:0}.links{font-size:13px}th,td{padding:12px;font-size:13px}.actions{display:grid}.btn{text-align:center}}</style></head><body><div class="layout"><aside class="side"><div class="brand"><div class="icon">☕</div><div><h1>كافيه فرفشة</h1><p>لوحة تحكم المدير</p></div></div><nav class="nav"><a class="active" href="/admin">📊 الرئيسية</a><a href="/">☕ فتح المنيو</a><a href="{{ qr_url }}" target="_blank">▦ رابط الباركود</a><a href="/logout">🚪 تسجيل خروج</a></nav></aside><main class="main"><div class="mobile-title"><h1>لوحة تحكم الطلبات ☕</h1></div><div class="top"><h2>نظرة عامة</h2><div class="top-actions"><a href="/">فتح المنيو</a><a href="{{ qr_url }}" target="_blank">QR</a><a href="/logout">تسجيل خروج</a></div></div><section class="grid"><div class="stat"><div class="circle">⏱️</div><div><b>{{ new_orders }}</b><p>طلبات قيد التحضير</p></div></div><div class="stat"><div class="circle">📋</div><div><b>{{ today_orders }}</b><p>إجمالي الطلبات اليوم</p></div></div><div class="stat"><div class="circle">💰</div><div><b>{{ sales }}</b><p>إجمالي المبيعات</p></div></div><div class="stat"><div class="circle">📍</div><div><b>{{ active_places }}</b><p>أماكن الطلب النشطة</p></div></div></section><div class="links"><div><b>رابط فتح المنيو:</b><br><a href="{{ menu_url }}">{{ menu_url }}</a></div><div><b>رابط الباركود:</b><br><a href="{{ qr_url }}">{{ qr_url }}</a></div></div><section class="panel"><h3>الطلبات الأخيرة</h3><p style="color:#7d6f67;margin:4px 0">عرض وإدارة جميع الطلبات</p><div class="filters"><input id="searchInput" onkeyup="filterOrders()" placeholder="بحث باسم العميل أو مكان الطلب أو رقم الطلب..."><select id="statusFilter" onchange="filterOrders()"><option value="">كل الحالات</option><option value="قيد التحضير">قيد التحضير</option><option value="تم التنفيذ">تم التنفيذ</option></select></div><div class="table-wrap"><table id="ordersTable"><thead><tr><th>رقم الطلب</th><th>اسم العميل</th><th>مكان الطلب</th><th>الوقت</th><th>العناصر</th><th>الإجمالي</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody>{% for order in orders %}<tr data-status="{{ order.status }}"><td>#{{ order.id }}</td><td><b>{{ order.customer_name }}</b></td><td>{{ order.order_place }}</td><td>{{ order.created_at }}</td><td>{% for item in order.order_items %}• {{ item["item_name"] }} × {{ item["qty"] }}<br>{% endfor %}</td><td><b>{{ order.total }}</b><br>جنيه</td><td>{% if order.status == "تم التنفيذ" %}<span class="badge done">● {{ order.status }}</span>{% else %}<span class="badge wait">● {{ order.status }}</span>{% endif %}</td><td><div class="actions"><a class="btn b-orange" href="/update-status/{{ order.id }}/قيد التحضير">تحضير</a><a class="btn b-green" href="/update-status/{{ order.id }}/تم التنفيذ">تم</a><a class="btn b-red" href="/delete-order/{{ order.id }}" onclick="return confirm('هل تريد حذف الطلب؟')">حذف</a></div></td></tr>{% endfor %}</tbody></table></div></section></main></div><script>function filterOrders(){let q=document.getElementById('searchInput').value.toLowerCase();let s=document.getElementById('statusFilter').value;document.querySelectorAll('#ordersTable tbody tr').forEach(r=>{let text=r.innerText.toLowerCase();let okText=text.includes(q);let okStatus=!s||r.dataset.status===s;r.style.display=(okText&&okStatus)?'':'none';});}{% if admin_notice %}alert({{ admin_notice|tojson }});{% endif %}</script></body></html>
'''
